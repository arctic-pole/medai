...
| `devices` | Phase 9: one row per registered patient device | `device_type`, optional `label`. `app/providers/devices/base.py`'s `DeviceAdapter` ABC has no Python implementation, and never will for a `device_type="health_connect"` row specifically — Health Connect has no cloud endpoint, so its real adapter is client-side (Phase 10, `mobile/lib/services/health_connect_adapter.dart`); this row exists so `POST /vitals/sync` has a `device_id` to validate ownership against. |
