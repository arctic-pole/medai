...
async def create_assessment(
    ...
    DETERMINISTIC_SAFETY -> OUTPUT_VALIDATION -> TEXT_RESPONSE, using Phase 3/5/6/7/8's
    ...
    MEDICATION_SAFETY gap: vitals are always {} here (Phase 9 — vital ingestion — doesn't exist
    yet, so DETERMINISTIC_SAFETY only ever evaluates against empty vitals, never a real
    reading). Medication safety is also not cross-checked against whatever this endpoint's own
    ...
    safety_evaluation = await evaluate_safety(db, patient_id=patient.id, vitals={})
